...
const findMedianSortedArrays2 = (nums1, nums2) => {
    ...
    while (idx1 + idx2 <= totalLength / 2) {
        ...
        if (elOne === undefined) {
            ...
            console.log(`elOne(${elOne}) < elTwo(${elTwo}), so current=${elOne} and idx1++`);
            current = elOne;
            idx1++;
        } else {
            console.log(`elOne(${elOne}) > elTwo(${elTwo}), so current=${elTwo} and idx2++`);
            current = elTwo;
            idx2++
        }
    }
    ...
    if (totalLength % 2 === 0) {
        ...
        console.log(`totalLength is even, so result = ${last} + ${current} / 2 = ${result}`);
    } else {
        result = current;
